...
let pendingResumeRequestCount = 0;
...
async function initDashboard() {
  ...
  try {
    ...
    await Promise.all([
      ...
      loadPendingResumeRequestCount(user.id),
...
async function loadPendingResumeRequestCount(userId) {
  const { count, error } = await candidateSupabase
  ...
  pendingResumeRequestCount = error ? 0 : count || 0;
}
...
function renderDashboard() {
  ...
  window.PlacelyCandidateSidebar?.updateVerificationStatus(profile.verification_status || "unknown");
  window.PlacelyCandidateSidebar?.updateResumeRequestCount(pendingResumeRequestCount);
  renderUtilityBadges(unreadMessagesCount, activeNotificationCount + pendingResumeRequestCount);
